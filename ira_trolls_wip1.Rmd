...
infomap <- cluster_infomap(g, nb.trials = 50,
                          modularity = F)

#running infomap on the troll subgraph
infomap.troll <- cluster_infomap(g.troll, nb.trials = 10,
                          modularity = F)

# running modularity optimization community detection
modularity <- cluster_louvain(g.undir)

#running modularity on the troll subgraph
modularity.troll <- cluster_louvain(g.troll.undir)
...
plot(g.troll,
     ...
     vertex.label.cex = .1,
     vertex.color = infomap.troll$membership)
...
print(xtable1, include.rownames = F)
##unnecessary?
#kable(xtable1, col.names=colnames)
#?kable()

#xtable(table1, caption = "Summary of Users", label = NULL, align = T, digits = NULL,
...
Category <- c("Right", "Left", "Hashtag Gamer", "Non-English", "Newsfeed", "Commercial", "Fearmonger", "Unknown", "Total")
N <- c(length(troll_nodes$account_category[troll_nodes$account_category=="RightTroll"]), length(troll_nodes$account_category[troll_nodes$account_category=="LeftTroll"]), length(troll_nodes$account_category[troll_nodes$account_category=="HashtagGamer"]), length(troll_nodes$account_category[troll_nodes$account_category=="NonEnglish"]),
       length(troll_nodes$account_category[troll_nodes$account_category=="NewsFeed"]),
       length(troll_nodes$account_category[troll_nodes$account_category=="Unknown"]), "404"
       )
Senders <- c()

table2 <- cbind(Category, N, , )
