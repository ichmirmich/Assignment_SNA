...
## for how many tweeting handles is an account type available?
...
453-333
## 120 trolls only tweeted and did not retweet
...
## 3101
## Shoudld'nt it be 2752, if all handles in "tweets$handle" are officially trolls? 
## "At the request of NBC News, three sources familiar with Twitter's data systems cross-referenced the list of names released by Congress, excluding any account that Twitter later restored, to create a partial database of tweets that could be recovered from the suspended accounts. The sources asked to remain anonymous to avoid any politicization of their work or being identified as possibly violating Twitter's developer policy." (NBC article)

2752+453 
## 3205, (if none of our sender handles occured in the list)

3205-3101 
## 104 of our handles are offical trolls following the list by the US congress

453-104
## 349 of our troll handles are not officially trolls following the list by the US congress???

###################################
## Who do we think are really trolls?
...
## there are 88 trolls in the retweet network

############################
## repeating the above, including the NBC sources' definition of trolls
handles_trolls$troll <- "troll"
nodes2 <- right_join(handles_trolls, handles_all)
length(na.omit(nodes2$troll)) 
## there are 389 trolls in the retweet network

nodes2 <- replace(nodes2, is.na(nodes2), "non-troll") ## now we have a variable indicating wether a user is a troll

nodes_categorized <- left_join(nodes2, account_type)
