...
rank.indegree <- nodes %>% select(handle, com, 
                                   indegree, indegree_weighted) %>% arrange(-indegree)
rank.indegree.w <- nodes %>% select(handle, com, 
...
w_indegree_table <- w_indegree_table %>% rename(User = handle, Community = com, 
                                                  Indegree = indegree,
                                                  Indegree_Weighted = indegree_weighted)
...
```{r}
## BOX PLOT
ggplot(troll_nodes, aes(x = com, y = indegree)) +
  geom_boxplot()
```
