...
## 147,428
rts.as.df / length(tweets$handle)
...
```{r, include = F}
## subsetting the graph by removing non-trolls; NOT WORKING!!!

## removing loops and multiples
rts.g <- simplify(rts.g, remove.multiple = T, remove.loops = T)

## g.troll <- induced.subgraph(rts.g, which(V(rts.g)$troll == "troll"))
...
V(g)$modularity.troll <- modularity.troll$membership
V(g)$infomap.troll <- infomap.troll$membership

df.community <- data.frame(handle = V(g)$name, infomap = V(g)$infomap, modularity = V(g)$modularity)
head(df.community)

df.community.troll <- data.frame(handle = V(g)$name, infomap = V(g)$infomap.troll, modularity = V(g)$modularity.troll)
head(df.community)
...
ggplot(troll_nodes, aes(x=account_category)) +
  ...
  xlab("Qualitative Account Categories") +
...
## merging the two right communities

## removing all communities below 50

#ggplot(troll_nodes, aes(x=modularity)) +
  #geom_bar(aes(fill = as.character(account_category)), show.legend=F 
           #) + 
  #ggtitle("Distribution of Communities within Account Categories by Linvill/Warren (2018)") +
  #xlab("Qualitative Account Categories") +
  #ylab("Number of Trolls") +
