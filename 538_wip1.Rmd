...
## Dieser Schritt gleicht die Twitter Handles mit der Liste tatsächlicher Trolls vom Congress ab. Wir gehen aber davon aus, dass alle Handles bereits Trolls sind -> kann deshalb übersprungen werden.
...
tweets_control <- tweets %>% rename(handle = author) #renaming handle name variable
handles <- tweets_control %>% select(handle) #selecting only the handles from the data
...
```{r, inlcude=F}
## Attention: Damit der nächste Code Chunk nach diesem funktioniert, müssen rt.sender und rt.receiver auf gleich Länge gebracht werden. Dazu werden hier die letzten 25 Reihen in rt.sender entfert.
